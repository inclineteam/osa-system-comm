...
const AddCommentForm = ({ comment, setComment, addComment, submittingComment = false }) => (
    <Form onSubmit={e => {
        e.preventDefault()
        addComment(comment)
    }}>
...
const CommentsView = ({ user, submissionBin, unitHead, className = "",report }) => {
    ...
    const addComment = (comment) => {
        ...
        axios.post('/comments/add', formData, MultipartHeader)
            .then((res) => {
                ...
                isFetchingComments ? (
                    ...
                    comments.length > 0 ? (
                        <>
                            ...
                            <div className="mt-2">
                                <AddCommentForm comment={comment} addComment={addComment} setComment={setComment} submittingComment={submittingComment} />
                            </div>
                        </>
                    ) : (
                        addingComments ? (
                            ...
                            <p className="text-sm cursor-pointer text-primary" onClick={() => setAddingComments(true)}>
