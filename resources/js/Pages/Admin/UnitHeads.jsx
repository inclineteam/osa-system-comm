...
import { Breadcrumb, BreadcrumbItem, Button, Card, Col, Form, Image, ListGroup, ListGroupItem, Nav, Row } from 'react-bootstrap'
                    ...
                    unitHead && (
                        <div className='px-3'>
                            <div className="flex items-end gap-3">
                                ...
                                <div>
                                    <p className='text-dark'>{unitHead.firstname} {unitHead.lastname}</p>
                                </div>
                            </div>
                            <p className="flex gap-2 mt-3 mb-0 text-sm ">
                                <span className=' fw-bold'><i className='fi fi-rr-school me-1'></i> Campus:</span>
                                <span className=' '> {unitHead.campus.name}</span>
                            </p>
                            <hr className='my-3' />
                            <p className="flex gap-2 mb-0 text-sm ">
                                <span className='text-secondar fw-bold'><i className='fi fi-rr-user me-1'></i> Designation:</span>
                                ...
                                <ListGroup variant="flush" className=' shadow-sm rounded-0 mb-3'>
                                    ...
                                    </ListGroup.Item>
                                    {
                                        campuses && campuses.map((campus, index) => (
                                            <ListGroup.Item onClick={() => setSelectedCampus(campus)} key={index} className={`cursor-pointer ${index == selectedCampusIndex ? ' text-primary fw-bold' : 'hover:bg-gray-50 text-secondary'}`}>
                        ...
                        <ListGroup variant="flush" className=' shadow-sm rounded-0'>
                            ...
                            </ListGroupItem>
                            {
                                classifications && classifications.map((item, index) => (
                                    <ListGroupItem onClick={() => setSelectedClassificationIndex(index)} key={index} className={`cursor-pointer ${index == selectedClassificationIndex ? 'text-primary fw-bold' : 'hover:bg-gray-50 text-secondary'}`}>
                                        <p className="my-1 flex justify-between items-center">
                                            <span>{item.name}</span>
                                            <i className='bx bx-chevron-right'></i>
                                        </p>
                                        ...
                                        designations && designations.map((designation, index) => (
                                            <>
                                                ...
                                                <HorizontalScrollingList
                                                    ...
                                                    selector={(item, index) => (
                                                        <button onClick={() => handleSelect(item)} className=' btn btn-link justify-content-center text-center'>
                                                            <div>
                                                                {
                                                                    item.image ? (
                                                                        <Image
                                                                        ...
                                                                        <TextProfilePic size='sm' text={item.firstname[0]} />
                                                                    )
                                                                }
                                                            </div>
                                                            <p className='my-0 text-sm fw-bold'>{item.firstname} {item.lastname}</p>
                                                        </button>
                                                    )}
                                                />
                                            </>
