...
export default function ForRequestedReports({ auth, reportsForRequested }) {
  return (
    <PanelLayout userAuth={auth} defaultActiveLink="User Events History">
      <div className="content-wrapper">
        <div className="p-4 border-b border-slate-300 rounded-lg shadow-sm bg-white">
          <h1 className="text-xl font-bold mb-2 leading-none">For requested</h1>
          <p className="leading-none mb-4 text-slate-500 text-sm">
            See all the reports that was requested.
          </p>

          {reportsForRequested.length > 0 ? (
            <div className="grid lg:grid-cols-2 xl:grid-cols-3">
              <div className="border rounded-lg border-slate-200">
                {reportsForRequested.map((report) => (
                  <div
                    key={report.id}
